...
function drawCircles(feature) {
    ...
    // let xMap = lerpPos[27].x;
    let xMap = map(lerpPos[27].x, 0, width, -2, 2);
    // let xMap = map(lerpPos[27].x, 0, width, -.50, .50);
    let yMap = map(lerpPos[27].y, 0, height, 0.50, -0.50);
    //   console.log(lerpPos[27].x);

    let mouth = createVector(map(lerpPos[62].x, 0, width, -2, 2), map(lerpPos[62].y, 0, height, 2, -2));
    ...
    window.globals.d = mouth.x;
    window.globals.e = mouth.y;
